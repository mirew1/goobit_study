...
!function(t) {
    ...
    h.ui = h.ui || {},
    h.ui.version = "1.12.1";
    var n, i = 0, a = Array.prototype.slice;
    ...
    h.widget = function(t, i, e) {
        var s, n, o, r = {}, a = t.split(".")[0], u = a + "-" + (t = t.split(".")[1]);
        return e || (e = i,
        i = h.Widget),
    ...
    h.widget.extend = function(t) {
        for (var e, i, s = a.call(arguments, 1), n = 0, o = s.length; n < o; n++)
            for (e in s[n])
                i = s[n][e],
                s[n].hasOwnProperty(e) && void 0 !== i && (t[e] = h.isPlainObject(i) ? h.isPlainObject(t[e]) ? h.widget.extend({}, t[e], i) : h.widget.extend({}, i) : i);
        return t
    }
    ,
...
!function(n) {
    n.widget("tl.drum", {
        ...
        _create: function() {
            ...
            this.element.watchDrag({
                dragstart: function(t, e) {
                    r.element.focus(),
                    "revolving" === r._state && (r._scrollToOffset(r._getCurrentOffset()),
                ...
                drag: function(t, e) {
                    r._scrollToOffset(r._drumOffset + e.dy),
                    r._trigger("drag", t, e)
                },
                dragend: function(t, e) {
                    var i, s, n, o = r.options.acceleration;
                    (i = Math.abs(e.vy)) ? ((n = Math.pow(i, 2) / (2 * o)) > r.options.maxSpinOffset && (n = r.options.maxSpinOffset),
                    n *= function(t) {
        ...
        _stopRevolving: function(t) {
            "standby" !== this._state && (this._refillValuesAroundView(),
        ...
        _render: function() {
            ...
            if (this._addClass(this.element, "drum-viewport"),
            ...
            "SELECT" === t.nodeName || this.options.options || "select" === this.options.type) {
                ...
                this.options.options)
                    ...
                    this._setOption("options", this._parseSelectMenuOptions(n(t).find("option")))
                }
                void 0 === this.options.value && (this.options.value = this.options.options[0].value)
            } else
                (this.options.min || this.options.max || "INPUT" === t.nodeName && "number" === t.type || "number" === this.options.type) && (this.options.type = "number",
                "INPUT" === t.nodeName && ("" !== t.min && (this.options.min = this._formatValue("min", t.min)),
        ...
        _formatValue: function(t, e) {
            ...
            case "max":
            case "step":
                "number" === this.options.type && (e = parseFloat(e),
                isFinite(e) || (e = void 0));
                break;
            case "type":
                if ("number" !== e && "select" !== e)
                    throw new Error('Wrong value for type: "' + e + '". The supported types are "number" and "select".');
                break;
            case "orderAsc":
            case "watchOutside":
                if ("boolean" != typeof e)
                    throw new TypeError(t + " must be boolean. " + typeof e + " passed.");
                break;
            case "options":
                if (!Array.isArray(e))
        ...
        _setOption: function(t, e, i) {
            ...
            if (void 0 === (e = this._formatValue(t, e)))
                throw new TypeError("Failed to set the " + t + " property on Drum: The provided value is non-finite.");
            if (-1 !== ["min", "max", "step"].indexOf(t) && "number" !== this.options.type)
                throw new TypeError("Failed to set the " + t + " property on Drum: " + t + " can only be set on widget type number.");
            if ("value" === t && "number" === this.options.type && (null != this.options.min && e < this.options.min && (e = this.options.min),
            null != this.options.max && e > this.options.max && (e = this.options.max)),
            !("min" === t && e >= this.options.max || "max" === t && e <= this.options.min))
                return "options" === t && (this.items = e.slice(),
                e.some(function(t) {
                ...
                "min" === t && this.options.value < e ? this._setOption("value", e) : "max" === t && this.options.value > e ? this._setOption("value", e) : void (-1 !== ["min", "max", "step", "value", "options"].indexOf(t) && (this._refillValuesAroundView("value" === t ? e : null),
                this._centerView(this._drumEl.find('[data-value="' + this.options.value + '"]')[0], i)))
        },
        _updateValues: function(t, e) {
            ...
            this._origControl && ("INPUT" !== this._origControl.nodeName || "value" !== t && "max" !== t && "min" !== t && "step" !== t || (this._origControl[t] = e),
            "SELECT" === this._origControl.nodeName && ("value" === t ? this._origControl.value = e : "options" === t && (this._origControl.innerHTML = e.map(function(t) {
        ...
        _scrollToOffset: function(t) {
            if (void 0 === t)
                throw new Error("You must pass an offset value");
        ...
        _getItemInView: function() {
            var t, e = this.element[0].getBoundingClientRect(), i = (e.left,
            e.width,
            e.top + e.height / 2), s = this._drumEl.children(), n = this._drumEl[0].getBoundingClientRect();
            return (t = s[Math.max(0, Math.min(s.length - 1, Math.floor((i - n.top) / n.height * s.length)))]).classList.contains("drum-item") || (t = 0 < this._drumOffset ? this._drumEl.children()[0] : this._drumEl.children().last()[0]),
            t
        },
        value: function(t) {
            if (void 0 === t)
        ...
        selectNext: function() {
            ...
            return !!t && (this._setOption("value", t.dataset.value),
            t)
        },
        selectPrev: function() {
    ...
    i.widget("tl.watchDrag", {
        ...
        _eventsDrag: {
            mousemove: function(t) {
                ...
                this._trigger("drag", t, e)
            },
            mouseup: function(t) {
                t.preventDefault(),
                t.stopPropagation();
                var e = i.extend({}, this._coords);
                Date.now() - this._coords.t <= this.options.minDragInterval ? (e.vx = e.dx / e.dt * 1e3,
                e.vy = e.dy / e.dt * 1e3) : e.vx = e.vy = 0,
