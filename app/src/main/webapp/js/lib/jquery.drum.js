...
$.widget('tl.drum', {
	...
	_drawItems: function(items) {
		...
		items.forEach(function(item) {
			var disabled = item.disabled == true ? 'disabled' : ''
			var itm_html = '<div class="drum-item '+disabled+'" data-value="' + item.value + '"    >' + item.label + '</div>';
	...
	_centerView: function(itemInView, animate) {
		...
		if(this._drumEl.find('.drum-item.drum-item-current').hasClass("disabled")){
			$(".drum-viewport").removeClass("disabled").addClass("disabled");
		}else {
			$(".drum-viewport").removeClass("disabled");
		}
